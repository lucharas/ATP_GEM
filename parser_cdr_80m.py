...
VAR_MAP = {
    't': 'tmp',   # Temperatura na 80m
    'q': 'sh',    # Wilgotność właściwa na 80m
    'u': 'u80',   # Wiatr zonalny na 80m
    'v': 'v80',   # Wiatr merydionalny na 80m
}
...
TARGET_COLS = ['yyyy', 'mm', 'dd', 'hh', 'lat', 'lon', 'tmp', 'sh', 'u80', 'v80']
...
def main():
    ...
    for hour in config.FCT_HOURS_CDR:
        fct_str = f"{hour:03d}"
        filename_in = f"CDR_80m_f{fct_str}.grib"
        ...
        try:
            ...
            # Temperatura (K) - 2 miejsca
            df_final['tmp'] = df_final['tmp'].round(2) 
            # Wilgotność właściwa (kg/kg) - 5 miejsc
            df_final['sh'] = df_final['sh'].round(5) 
            # Wiatr (m/s) 
            df_final['u80'] = df_final['u80'].round(4) 
            df_final['v80'] = df_final['v80'].round(4)
