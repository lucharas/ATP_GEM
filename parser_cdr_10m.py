...
VAR_MAP = {
    'u10': 'u10',   # Wiatr zonalny na 10m
    'v10': 'v10',   # Wiatr merydionalny na 10m
}
...
TARGET_COLS = ['yyyy', 'mm', 'dd', 'hh', 'lat', 'lon', 'u10', 'v10']
...
def main():
    ...
    for hour in config.FCT_HOURS_CDR:
        fct_str = f"{hour:03d}"
        filename_in = f"CDR_10m_f{fct_str}.grib"
        ...
        try:
            ...
            # 9. Zaokrąglanie wartości (wiatr - 2 miejsca po przecinku)
            df_final['lat'] = df_final['lat'].round(3)
            df_final['lon'] = df_final['lon'].round(3)
            df_final['u10'] = df_final['u10'].round(4) 
            df_final['v10'] = df_final['v10'].round(4) 
